    ...
    <div className="flex flex-col h-screen w-screen overflow-hidden bg-[#0a0a12] text-white">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-col flex-1 overflow-hidden min-w-0">
          ...
          {/* ── D1–D10 stepper ── */}
          <div className="shrink-0 px-4 py-3 border-b border-white/8 overflow-x-auto">
            <div className="flex items-start justify-between min-w-max gap-0 relative">
              {/* track */}
              ...
              {ALL_DIMS.map((dim, idx) => {
                ...
                return (
                  <div key={dim} className="relative flex flex-col items-center" style={{ width: 88 }}>
                    {/* circle */}
                    <div className={`relative z-10 w-9 h-9 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${
                      isActive
                        ? 'bg-indigo-600 border-indigo-600 shadow-[0_0_0_4px_rgba(99,102,241,0.25)]'
                        : isDone
                      ...
                      {isDone
                        ...
                        : isActive
                        ? <span className="text-white text-xs font-bold">{dim.replace('D','')}</span>
                        : <span className="text-white/30 text-xs font-semibold">{dim.replace('D','')}</span>
                      }
                    </div>
                    {/* label */}
                    <div className="mt-2 text-center">
                      <p className={`text-xs font-semibold leading-none ${isActive ? 'text-indigo-400' : isDone ? 'text-white/55' : 'text-white/25'}`}>{dim}</p>
                      <p className={`text-[11px] mt-1 leading-none ${isActive ? 'text-indigo-300/80' : isDone ? 'text-white/40' : 'text-white/20'}`}>{DIM_LABELS[dim][language]}</p>
                      {isDone && cov && (
                        <div className="mt-1.5 w-10 mx-auto h-0.5 rounded-full bg-white/8 overflow-hidden">
